...
import { musicGenre, listenStories } from '../StoriesData/Data';

const ListenPage = () => {

    return ( 
        <div className="Listen absolute inset-0 bg-black h-fit w-full">
            ...
            <div className="w-full px-[60px]">
                <div className="w-full flex flex-wrap justify-center gap-3 my-[20px]">
                    {musicGenre.map((genre, index) => (
                        <div key={index} className="px-[4px] border-2 border-solid border-[#CCF566] text-[#CCF566] text-[14px] whitespace-nowrap">{genre}</div>
                    ))}
                </div>
            </div>
